...
// return getAuthorizationCookie(serverHttpRequest) ;//.filter(t ->
// t.startsWith(jwtConfig.getPrefix())).map(t -> t.replace(jwtConfig.getPrefix(), ""));
// }
...
// return
// Optional.ofNullable(cookies.getFirst(HttpHeaders.AUTHORIZATION)).map(HttpCookie::getValue);
// }
